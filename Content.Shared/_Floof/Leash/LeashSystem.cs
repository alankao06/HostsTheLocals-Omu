...
public sealed class LeashSystem : EntitySystem
{
    ...
    {
        //YES YOU CAN LEASH ME!!
        //The seven guard clauses
        if (_net.IsClient)
            return false;
        ...
        if (!TryComp<HandsComponent>(player, out var hands))
            return false;
        if (_hands.TryGetActiveItem(player, out var leash))
            return false;
        if (!TryComp<LeashComponent>(leash, out var leashComp))
            return false;
        if (leashComp.NextPull > _timing.CurTime)
            return false;
        if (leashComp.NextPull > _timing.CurTime)
            return false;
    ...
    private bool TryGetLeashTarget(Entity<LeashAnchorComponent?> ent, out EntityUid leashTarget)
    {
        ...
        && _container.TryGetContainingContainer(Transform(ent.Owner).ParentUid, ent.Owner, out var container))
        {
